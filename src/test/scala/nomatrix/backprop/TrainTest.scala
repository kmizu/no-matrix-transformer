...
class LossTest extends munit.FunSuite:
  ...
  test("交差エントロピーは -log(softmax の正解確率)") {
    val logits = GVec.fromDoubles(Seq(1.0, 2.0, 3.0))
  ...
  test("大きなロジットでも NaN にならない") {
    val logits = GVec.fromDoubles(Seq(1000.0, -1000.0))
  ...
  test("列の損失は各位置の平均") {
    val a = GVec.fromDoubles(Seq(1.0, 0.0))
    val b = GVec.fromDoubles(Seq(0.0, 1.0))
