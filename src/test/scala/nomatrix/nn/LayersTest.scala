...
class LayersTest extends munit.FunSuite:
  ...
  test("Neuron は重み付き和 + バイアス") {
    val n = Neuron(Vec.fromDoubles(Seq(1.0, 2.0)), Value(0.5))
    assertEqualsDouble(n(Vec.fromDoubles(Seq(3.0, 4.0))).data, 11.5, 1e-9)
  }
  ...
  test("LayerNorm はゲイン 1・バイアス 0 なら Vec.layerNorm と同じ") {
    ...
    val x = Vec.fromDoubles(Seq(1.0, 2.0, 3.0, 4.0))
    assertEquals(Vec.data(ln(x)), Vec.data(Vec.layerNorm(x)))
  }
  ...
  test("AttentionHead は因果的: 未来のトークンを変えても過去の出力は変わらない") {
    ...
    val xs1 = Vector(Seq(1.0, 0.0, 0.0, 1.0), Seq(0.0, 1.0, 1.0, 0.0), Seq(0.5, 0.5, 0.5, 0.5)).map(Vec.fromDoubles)
    val xs2 = xs1.updated(2, Vec.fromDoubles(Seq(-3.0, 2.0, 9.0, -1.0)))
    val o1 = head(xs1).map(Vec.data)
    val o2 = head(xs2).map(Vec.data)
  ...
  test("AttentionHead: 1 トークンだけなら出力は value 変換そのもの") {
    ...
    val x = Vec.fromDoubles(Seq(0.1, 0.2, 0.3))
    val v = Dense.load(pv, "h.value", in = 3, out = 3)(x)
    assertEquals(Vec.data(head(Vector(x))(0)), Vec.data(v))
  }
  ...
  test("FeedForward は dModel -> hidden -> dModel") {
    ...
    assertEquals(ff(Vec.fromDoubles(Seq(1.0, 2.0, 3.0))).length, 3)
  }
  ...
  test("Block は形を保ち、残差があるので入力ゼロでもゼロで潰れない") {
    ...
    assert(out.forall(_.length == 4))
    assert(Vec.data(out(0)).exists(_ != 0.0))
  }
