...
それでも、当てずっぽうの 4.16 から、微分を一度も使わずに 1.02 まで下がりました。付録の逆伝播（2000 ステップ、4 分）が 1.01 なので、届く場所はほぼ同じです。時間は 15 倍かかっています。
...
!!! tip "この章のまとめ"
    - 当てずっぽうの損失 \( \log 64 \approx 4.16 \) から、微分なしの 8000 ステップで 1.02 まで下がる（逆伝播とほぼ同じ場所）
