...
本線は微分を使わずに学習しました。この付録では、同じモデルを**微分（逆伝播）**で学習します。
第14章で書いたとおり、微分は「全部の方向を一度に試す」高速化の手段です。
ここでは、それを行列なしで実装します。
...
本線の数は `Double` でした。「少し動かしたら損失がどう変わるか」は、実際に動かして測るしかありません。

ここでは、**計算しながら「自分がどこからどれだけの傾きで作られたか」を記録する数**を作ります。これが `Value` です。

たとえば \( c = a \times b \) なら、\( a \) を少し動かすと \( c \) は \( b \) 倍動きます。
...
本線の方法（実際に少し動かして差を取る）と、逆伝播の答えが一致することを確かめます。
...
逆伝播は 1 回の計算で「すべての入力について」の傾きが出ることです。
...
!!! tip "この付録のまとめ"
    - `Value` = 値 + 「誰からどれだけの傾きで作られたか」。逆伝播で全パラメータの傾きが一度に求まる
    - 部品の構造は本線と同じ。数の種類が違うだけ
    - 微分は「動かして測る」の高速化であり、答えは同じ
