...
## 追記（2026-09-24 午後）: 本線を「微分なし」に張り替え

コウタの提案「微分すら使わずに説明できないか」を受けて実験し、本線を微分なしの学習に切り替えた。

- 本線: `Double` だけの層（`nomatrix.vec` / `nn` / `model`）と、ゆらぎ学習 `nomatrix.train.Evolution`
  （対称なゆらぎ ±σε を pairs 組試し、損失差でゆらぎを重み付けした平均を「向き」として Adam で歩幅調整）。
- 付録: 旧本線（スカラー自動微分 `Value` と逆伝播）を `nomatrix.backprop` に移した。`EquivalenceTest` が
  同じパラメータで本線と 1e-9 まで同じ損失を出すことを固定している。
- 学習では配列版の順伝播 `FastTransformer` を使う（読みやすい版の 4.5 倍速、同じロジットをテストで確認）。
- 実測（6,848 パラメータ、評価は固定 64 窓）: 逆伝播 2000 ステップ 4 分で損失 1.01。ゆらぎ学習は
  64 対・σ=0.02・lr=0.003・8 窓で 4000 ステップ 31 分で 1.39、8000 ステップで 1.2 前後の見込み。
- 章立て: 2 数の並び / 3 ニューロン / 4 埋め込み / 5 位置 / 6 注意 / 7 Multi-Head / 8 ブロック / 9 Transformer /
  10 損失 / 11 学習（微分なし） / 12 生成 / 13 実験 / 14 行列と微分はどこにいたのか / 付録 微分で速くする。
